...
class Classifier(Network):
    ...
    def forward(self, X):
        ...
        self.cache['X'] = X
        # print(np.shape(X))
        z = X*np.transpose(self.W) # compress z into shape N x 1
        z = self.sigmoid(np.sum(z, 1)).reshape(-1,1)
        # print(np.shape(z))
        self.cache['Sig'] = z
    ...
    def backward(self, dout):
        ...
        # dout N x 1
        dsigmoid = self.cache['Sig']*(1-self.cache['Sig']) # N x 1
        dwx = self.cache['X'] # N x (D + 1)
        # dW (D + 1) x 1
        # print('dsigmoid:', np.shape(dsigmoid))
        # print('dout:', np.shape(dout))
        # print('dwx:', np.shape(dwx))
        dW = dwx*dout*dsigmoid
        dW = np.sum(dW, 0).reshape(-1, 1)
        # dW = np.transpose(dW)
