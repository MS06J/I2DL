...
class RescaleTransform:
    ...
    def __call__(self, image):
        ...
        ret_image = image
        in_range = self._data_max - self._data_min
        out_range = self.max - self.min

        ret_image = ((ret_image-self._data_min)*out_range/in_range) + self.min
...
def compute_image_mean_and_std(images):
    ...
    # num_channels = np.shape(images)[3]
    # print(num_channels)
    # mean = np.reshape(images.mean(3), num_channels)

    # std = np.reshape(images.std(3), num_channels)
